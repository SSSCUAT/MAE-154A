...
DETECTION_RANGE = 3000;       % Range to begin intercept [m]
CAPTURE_RADIUS  = 50;         % Capture distance [m]

% Velocity limits
V_CRUISE = 130;               % Cruise speed [m/s]
V_MIN    = 80;                % Minimum allowed speed
V_MAX    = 180;               % Maximum allowed speed
V_SLOW_RADIUS = 500;          % Radius for slowing down near target

% Altitude commands
ALT_SEARCH    = 1000;         % Altitude during waypoint search
ALT_INTERCEPT = 500;          % Altitude during intercept
ALT_CLIMB     = 2000;         % Altitude during climb-out
...
%% ================= MODE TRANSITIONS =================
% SEARCH -> INTERCEPT
if mode == 1 && range < DETECTION_RANGE
    mode = 2;
end
...
if mode == 2
    ...
    if range < V_SLOW_RADIUS
        vCommand = max(V_MIN, V_CRUISE * range / V_SLOW_RADIUS);
    else
